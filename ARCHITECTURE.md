...
- Expiration: upload accepts `ttl` (60s–30d, default none) → `expires_at`. Lazy sweep on access purges immediately, but a guaranteed janitor (`purgeExpired` in `src/lib/purge.ts`) makes expiry real for never-requested files: it deletes the Telegram blob + row for expired files and drops soft-deleted rows past a 7-day retention. Runs from `GET /api/cron/purge` (Vercel cron in `vercel.json`, once daily, optional `CRON_SECRET`) and probabilistically from ~1 in 20 uploads, so self-heals even without cron. Bounded at 50 rows/run.
